...
import type {
  Customer, CustomerLicense, PaymentStatus, LicenseStatus, RenewalType,
} from "@/lib/types";
...
export default function CustomerEditorClient({ customer, licenses: initialLicenses, isNew, userId }: Props) {
  ...
  const handleSave = useCallback(async () => {
    ...
    for (const l of licenses) {
      if (!l.product_name.trim()) errs[l.id] = "Product name is required.";
    }
    if (Object.keys(errs).length > 0) { setLicenseErrors(errs); return; }
    ...
    try {
      ...
      // ── Sync licenses ────────────────────────────────────
      // Delete all existing and re-insert (simplest approach for this UI pattern)
      if (!isNew) {
        ...
        <section className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Field label="Card Type">
              <SelectInput value={cardType} onChange={setCardType}>
            ...
            <Field label="Last 4 Digits">
              <TextInput value={cardLast4} onChange={(v) => setCardLast4(v.replace(/\D/g, "").slice(0, 4))} placeholder="1234" />
            </Field>

            <Field label="Expiry Month">
              <SelectInput value={cardExpiryM} onChange={setCardExpiryM}>
            ...
            <Field label="Expiry Year">
              <SelectInput value={cardExpiryY} onChange={setCardExpiryY}>
            ...
            <Field label="Payment Processor Reference" className="md:col-span-2">
              <MaskedInput value={processorRef} onChange={setProcessorRef} placeholder="e.g. cus_abc123 (Stripe customer ID)" />
              <p className="text-xs text-gray-600">Store the external customer/subscription ID from Stripe, Braintree, etc.</p>
            </Field>
          </div>
        </section>
          ...
          {licenses.length === 0 ? (
            ...
            <div className="flex flex-col gap-4">
              {licenses.map((lic) => {
                ...
                return (
                  <div key={lic.id} className={`border rounded-xl p-4 flex flex-col gap-3 ${licenseErrors[lic.id] ? "border-red-500/40 bg-red-500/5" : "border-gray-700 bg-gray-800/50"}`}>
                    {/* Row 1: product + status + delete */}
                    <div className="flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <input
                          type="text"
                          value={lic.product_name}
                          onChange={(e) => updateLicense(lic.id, { product_name: e.target.value })}
                          placeholder="Product name *"
                          className="w-full bg-transparent border-b border-gray-600 focus:border-indigo-500 py-0.5 text-sm font-medium text-white placeholder-gray-500 focus:outline-none transition-colors"
                        />
                        {licenseErrors[lic.id] && (
                          <p className="text-xs text-red-400 mt-1">{licenseErrors[lic.id]}</p>
                        )}
                      </div>
                      <span className={`shrink-0 px-2.5 py-0.5 rounded-lg border text-xs font-semibold ${meta.bg} ${meta.color}`}>
                        {meta.label}
                      </span>
